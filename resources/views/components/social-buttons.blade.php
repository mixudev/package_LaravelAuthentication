...
Deskripsi: Komponen tombol login pihak ketiga otomatis berbasis Socialite (Light & Dark).
=============================================================================
--}}
@php
    ...
    <div class="space-y-3">
        <div class="grid grid-cols-1 sm:grid-cols-{{ ($googleEnabled && $githubEnabled) ? '2' : '1' }} gap-3">
            ...
            @if ($googleEnabled)
                <a 
                    href="{{ $googleUrl }}" 
                    class="flex items-center justify-center gap-2.5 px-4 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white text-xs font-medium transition-all shadow-xs focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                >
