{{-- 
=============================================================================
KOMPONEN: TOMBOL SOCIAL AUTH (CLEAN BREEZE STYLE)
...
@if ($googleEnabled || $githubEnabled)
    <div class="space-y-2">
        <div class="grid grid-cols-1 sm:grid-cols-{{ ($googleEnabled && $githubEnabled) ? '2' : '1' }} gap-2">
            
            @if ($googleEnabled)
                <a 
                    href="{{ $googleUrl }}" 
                    class="flex items-center justify-center gap-2 px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 text-sm font-medium transition shadow-sm"
                >
