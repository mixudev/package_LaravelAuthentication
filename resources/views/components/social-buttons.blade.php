{{-- 
=============================================================================
KOMPONEN: TOMBOL SOCIAL AUTH (OAUTH GOOGLE & GITHUB)
Package: mixudev/laravel-authentication
Deskripsi: Komponen tombol login pihak ketiga otomatis berbasis Socialite.
           Tombol hanya muncul jika fitur social login & provider aktif pada config.
=============================================================================
--}}
@php
...
@if ($googleEnabled || $githubEnabled)
    <div class="space-y-3">
        <div class="grid grid-cols-1 sm:grid-cols-{{ ($googleEnabled && $githubEnabled) ? '2' : '1' }} gap-3">
            
            {{-- Tombol Google OAuth --}}
            @if ($googleEnabled)
                <a 
                    href="{{ $googleUrl }}" 
                    class="flex items-center justify-center gap-2.5 px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-800 hover:border-slate-700 text-slate-200 hover:text-white text-xs font-medium transition-all shadow-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                >
